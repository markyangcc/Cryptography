...
const int Nr = 16; // AES-256需要 16 轮加密
...
byte Inv_S_Box[16][16] = {{0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5, 0x38, 0xBF,
...
word Rcon[16] = {0x0f, 0x15, 0x71, 0xc9, 0x47, 0xd9, 0xe8, 0x59,
...
int main() {
  ...
  ifstream in;
  ofstream out;
  in.open("fly.txt", ios::binary);
  out.open("cipher.dat", ios::binary);
  ...
  // 解密并写入fly1.txt
  in.open("cipher.dat", ios::binary);
  out.open("fly1.txt", ios::binary);
  while (in.read((char *)&data, sizeof(data))) {
    divideToByte(plain, data);
    decrypt(plain, w);
    data = mergeByte(plain);
  ...
  in.close();
  out.close();
  cout << "Encrpt and decrypt file \"fly.txt\", OK." << endl;
