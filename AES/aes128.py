...
class AES:
    ...
    I_MIXC = [[0xe, 0xb, 0xd, 0x9], [0x9, 0xe, 0xb, 0xd],
    ...
    def key_expandsion(self, _16bytes_key):
        # 轮密钥产生
        w = [_16bytes_key >> 96,
        ...
        验证密钥扩展
        '''
        print("---------------------Key expandsion--------------------------------\n")
        count = 0
        for i in range(11):
            print(f'w[{count}]={hex(w[4*i])}', end='  ')
            count += 1
            print(f'w[{count}]={hex(w[4*i+1])}', end='  ')
            count += 1
            print(f'w[{count}]={hex(w[4*i+2])}', end='  ')
            count += 1
            print(f'w[{count}]={hex(w[4*i+3])}', end='  ')
            count += 1
            print("\n")
        print("------------------------------------------------------------------")
        print("\n\n")
    ...
    def aes_encrypt(self, plaintext_list, RoundKeys):
        ...
        print("At first, state: ")
        self.print_bytes_hex(State)
        print("\n\n")

        # 第一次 AddRoundKey
        State = self.AddRoundKey(State, RoundKeys, 0)
        print('At first, after AddRoundKey, State: ')
        self.print_bytes_hex(State)
        print("\n\n")
        ...
        for Round in range(1, 10):
            ...
            print(f'Round --{Round}-- after SubBytes, State: ')
            self.print_bytes_hex(State)

            State = self.ShiftRows(State)
            print(f'Round --{Round}-- after ShiftRows, State: ')
            self.print_bytes_hex(State)

            State = self.MixColumns(State)
            print(f'Round --{Round}-- after MixColumns, State: ')
            self.print_bytes_hex(State)

            State = self.AddRoundKey(State, RoundKeys, Round)
            print(f'Round --{Round}-- after AddRoundKey, State: ')
            self.print_bytes_hex(State)
        print("\n\n")
...
if __name__ == '__main__':
    ...
    RoundKeys = aes.key_expandsion(key)
